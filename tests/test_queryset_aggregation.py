...
class TestAggregateMultiple:
    async def test_all_in_one_call(self, qs):
        ...
        assert set(result.keys()) == {"total", "total_rating", "avg_rating", "min_rating", "max_rating"}
...
class TestAggregateFiltered:
    ...
    async def test_chained_filters(self, qs):
        result = await (
            qs.filter(author_id=2)
            .filter(rating__gt=2.0)
            .aggregate(n=Count())
        )
        assert result["n"] == 1
 
    async def test_rating_range_filter(self, qs):
        result = await qs.filter(rating__gte=2.0).aggregate(
            n=Count(), hi=Max("rating")
        )
...
class TestCountAggregate:
    ...
    async def test_count_distinct(self, qs):
        ...
        assert result["n"] == 5  # three unique categories
