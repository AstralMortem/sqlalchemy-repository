...
class TestValues:
    ...
    async def test_values_with_annotate_count(self, qs):
        ...
        results = await qs.values("author_id").annotate(n=Count("id")).order_by("author_id").all()
    ...
    async def test_values_with_annotate_avg(self, qs):
        results = await (
            qs.values("author_id").annotate(total_rating=Avg("rating")).order_by("author_id").all()
        )
    ...
    async def test_values_with_annotate_filtered(self, qs):
        results = await (
            qs.filter(author_id=1).values("author_id").annotate(total=Sum("rating")).all()
        )
